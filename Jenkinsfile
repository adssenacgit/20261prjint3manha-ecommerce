pipeline {
    ...
    stages {
        stage('Verificar Repositório') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/main']], useRemoteConfigs: [[url: 'https://github.com/adssenacgit/20261prjint3manha-ecommerce']]])
            }
        }
        ...
        stage('Construir Imagem Docker') {
            steps {
                script {
                    def appName = 'backend-prjint3-ecommerce'
        ...
        stage('Fazer Deploy') {
            steps {
                script {
                    ...
                    sh "docker run -d --name ${appName} -p 4003:4003 ${imageTag}"
                }
            }
        }
    }
